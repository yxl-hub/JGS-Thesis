...
\titlespacing*{\section}
    ...
    {2\baselineskip}     % 2. 段前间距 (space before)
    {1\baselineskip}     % 3. 段后间距 (space after)
...
This is first table % 用的是自带的tabular*宏包，用于自动对齐文本宽度（而宏包tabular仅根据内容调整表格宽度）。内容无需换行或内容短优先使用
\begin{table}[htbp] % 这里的htbp表示浮动位置，h=here, t=top, b=bottom, p=page of floats
\centering % 表格居中
\caption{An Example of a Well-Formed Table} % 表格标题
\label{tab:tab 1} % 用于文中引用表格
...
This is second table % 用的是tabularx宏包，用于自动调整列宽。存在长内容优先使用
\begin{table}[htbp]
\centering
